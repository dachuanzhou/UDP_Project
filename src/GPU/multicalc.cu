...
__global__ void calc_func(const int ele_emit_id, float *image_data,
    ...
    if (image_x_id < N && image_y_id < N && recv_center_id < 2 * M) {
        ...
        for (int step_offset = 0; step_offset < parallel_emit_sum;
            ...
            bool is_valid =
                (diff < compare_value) || (diff > (2048 - compare_value));
            ...
            if (is_valid) {
                ...
                if (((num + middot + (OD - 1 - 1) / 2) > 100) &&
                    ...
                    float angle = acosf((radius * radius +
                                         dis_snd_to_sample * dis_snd_to_sample -
                                         imagelength * imagelength) /
                                        2 / radius / dis_snd_to_sample);
                    if ((angle < PI / 9)) {
                        sum_image +=
                            trans_sdata[(num + middot + (OD - 1 - 1) / 2) *
                                            ELE_NO +
                                        recv_id +
        ...
        if (cacheIndex == 0) {
            int pixel_index =
                image_y_id + image_x_id * image_y_dim; //线程块的索引
...
void get_ele_position(float *ele_coord_x, float *ele_coord_y) {
    ...
    for (int i = 256; i < 512; i++) {
        ele_coord_x[i] =
            rfocus * cos(first_one + (i - 256) * ele_angle + PI / 4);
        ele_coord_y[i] =
            -rfocus * sin(first_one + (i - 256) * ele_angle + PI / 4);
    }
    for (int i = 512; i < 768; i++) {
        ele_coord_x[i] =
            rfocus * cos(first_one + (i - 512) * ele_angle + PI / 2);
        ele_coord_y[i] =
            -rfocus * sin(first_one + (i - 512) * ele_angle + PI / 2);
    }
...
int main(int argc, char const *argv[]) {
    ...
    default:
        ...
        std::cout << "[parallel emit sum] [filter path] [bin path]"
                  << std::endl;
        std::cout
            << "[parallel emit sum] [filter path] [bin path] [output path]"
            << std::endl;
        exit(-1);
        break;
    }
    ...
    if (cudaMemcpyToSymbol(dev_ele_coord_x, ele_coord_x,
        ...
        cout << "ERROR :: Failed for cudaMemcpyToSymbol dev_ele_coord_x."
             << endl;
        return -1;
    }
    ...
    if (cudaMemcpyToSymbol(dev_ele_coord_y, ele_coord_y,
                           sizeof(float) * ELE_NO) != cudaSuccess) {
        cout << "ERROR :: Failed for cudaMemcpyToSymbol dev_ele_coord_y."
             << endl;
        return -1;
    }
    ...
    cudaStatus = cudaMalloc((void **)(&dev_filtered_data),
                            length_of_data_in_process * 2);
    ...
    // for (i=1;i<=1;i++)
    {
        ...
        cudaStatus = cudaMemcpy(
            dev_data_samples_in_process, &bin_buffer[bin_buffer_index],
        ...
        cudaStatus =
            precalcWithCuda(dev_data_samples_in_process, ele_emit_id,
                            dev_sumdata, dev_sumpoint, dev_filtered_data,
                            dev_imagedata, dev_pointcount, parallel_emit_sum);
        //}
        // over=time(NULL);
