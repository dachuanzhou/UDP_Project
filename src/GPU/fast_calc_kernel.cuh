...
// gridParam: <recv_group_id, NSAMPLE_group_id, parallel_emit_sum>
...
__global__ void fast_calc_kernel(      //
  ...
  // pixels_offset 
  // the following commented code lines achive same result,
  // but slower due to GPU arthitecture
  ...
  // to skip unnecessary calculation
  // "if(...) return" will cause bugs, use this method instead
  if (valid_flag) {
    ...
    // for loop size is roughly equal for each GPU computing block
    for (int recv_id_iter = beg_recv_id; recv_id_iter < end_recv_id;
      ...
      // OVERALL NOTE: since convolution step use R(x) = sum f(x + k) * g^reverse(k)
      // instead of original R(x) = sum f(x - k) * g(k)
      // adjust R accordingly. (difference: OD - 1)

      // NOTE: use -OD/2 instead of +OD/2 - 1
      const int magic = waves + MIDDOT - OD / 2;
      // NOTE: use -OD+1 instead of NONE
      if (magic > 100 - OD + 1 && magic <= POINT_LENGTH - OD + 1) {
...
void fast_calc(                        //
  ...
  dim3 grid_param(PIC_RESOLUTION / 32, PIC_RESOLUTION / 32, sender_id_group_size);
